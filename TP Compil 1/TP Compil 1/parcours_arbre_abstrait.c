...
void parcours_varDec(n_dec *n)
{
  if(rechercheDeclarative(n->nom)!=-1){
    erreur("identifiant de var deja utilise");
  }
  switch(portee){
...
{
  if(rechercheDeclarative(n->nom)!=-1){
    erreur("identifiant de tab deja utilise");
  }
  switch(portee){
    case P_VARIABLE_GLOBALE : 
      ...
      break;
    case P_VARIABLE_LOCALE :
      erreur("Mauvaise portee pour un tableau . Tableau doit etre globale.");
      break;
    case P_ARGUMENT :
      erreur("Mauvaise portee pour un tableau . Tableau doit etre globale.");
      break;
...
{
  if(rechercheDeclarative(n->nom)==-1){
    erreur("identifiant de tab non cree");
  }
